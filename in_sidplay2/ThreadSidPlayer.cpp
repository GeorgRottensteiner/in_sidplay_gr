...
void CThreadSidPlayer::SaveConfigToFile( PlayerConfig *plconf, wchar_t* fileName )
{
  ...
  if ( ( !plconf->useSongLengthFile ) || ( plconf->songLengthsFile.empty() ) ) outFile << "SongLengthsFile=" << "" << std::endl;
  else outFile << "SongLengthsFile=" << plconf->songLengthsFile << std::endl;
  outFile << "UseSTILFile=" << plconf->useSTILfile << std::endl;
  if ( plconf->hvscDirectory.empty() ) plconf->useSTILfile = false;
  if ( !plconf->useSTILfile ) outFile << "HVSCDir=" << "" << std::endl;
  else outFile << "HVSCDir=" << plconf->hvscDirectory << std::endl;
  outFile << "UseSongLengthFile=" << plconf->useSongLengthFile << std::endl;
...
void CThreadSidPlayer::SetConfig( PlayerConfig* newConfig )
{
  ...
  //open STIL file
  if ( ( m_playerConfig.useSTILfile ) && ( !m_playerConfig.hvscDirectory.empty() ) )
  {
    ClearSTILData();
    FillSTILData();
    FillSTILData2();
...
{
  std::map<const char*, char*, ltstr>::iterator i;
  char* stilFileName;
...
{
  std::map<const char*, std::vector<StilBlock*>, ltstr>::iterator i;
  char* stilFileName;
...
{
  std::map<const char*, char*, ltstr>::iterator it = m.begin();
  while ( it != m.end() )
  {
  ...
  std::map<const char*, std::vector<StilBlock*>, ltstr>::iterator it2 = m_stillMap2.begin();
  while ( it2 != m_stillMap2.end() )
  {
    const char *x = it2->first;
    std::vector<StilBlock*> y = it2->second;
    delete[] it2->first;
    for ( std::vector<StilBlock*>::iterator it3 = y.begin(); it3 != y.end(); ++it3 )
    {
      if ( ( *it3 ) != NULL )
      {
        delete *it3;
